...
export class ContactsService {
  ...
  getContact(id: string): Observable<Contact | undefined> {
    return this.http.get<Contact>(`api/contacts/${id}`)
      //.pipe(map(c => { c.dateOfBirth = c.dateOfBirth?.split('T')[0] ?? null; return c; }));
      //string type: '1994-05-05T00:00:00.000Z' => '1994-05-05' - this is needed to set the value of the date input in the edit form, which expects a string in the format 'yyyy-MM-dd' (see https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/date#value)
      .pipe(map(c => {
